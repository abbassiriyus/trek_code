...
router.post("/orders", auth.authenticateToken, async (req, res) => {
  ...
  const query =
    "INSERT INTO orders (trek_id, sender) VALUES ($1, $2) RETURNING *";
...
router.get("/orders/search", async (req, res) => {
  try {
    ...
    const users =
      "SELECT address,email,id,firstname,patronimic,lastname FROM users";
    ...
    for (let i = 0; i < result1.rows.length; i++) {
      for (let j = 0; j < result2.rows.length; j++) {
        if (
          result1.rows[i].id == result2.rows[j].orders_id &&
          result2.rows[j].insender[0]
        ) {
...
router.get("/myorders", auth.authenticateToken, async (req, res) => {
  try {
    ...
    var a = result1.rows.filter((item) => {
      item.sender === req.user.userId;
    });
    res.json(result1.rows);
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
